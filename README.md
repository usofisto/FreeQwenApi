...
Исходный FreeQwenApi создавал **новый чат на каждый запрос** — даже когда клиент отправлял продолжение диалога. Это ломало контекст и тратило лимиты.

### Что исправлено:

1. **Переиспользование чатов** — сессии теперь сохраняются на диск (`session/sessions.json`, `session/chat-mapping.json`). При перезапуске сервера чаты не теряются, повторные запросы от одного клиента идут в тот же чат на Qwen.

2. **`ALLOW_UNSCOPED_SESSION_CHAT_RESTORE` включён по умолчанию** — даже без передачи `conversation_id` клиентом, прокси генерирует стабильный chatId на основе IP+User-Agent и переиспользует его.

3. **TTL сессий увеличен с 1 часа до 24 часов** — не нужно пересоздавать чат каждый час.

4. **Persistent chat ID mapping** — маппинг между внутренними ID и реальными Qwen chatId теперь не теряется при рестарте.

### Как работает:

Клиент (Open WebUI, Hermes, curl) → `POST /api/chat/completions` → прокси ищет существующий чат в сессии → если есть — отправляет в него, если нет — создаёт новый и запоминает.

### Сброс сессий:

Удалите файлы `session/sessions.json` и `session/chat-mapping.json` чтобы сбросить все сессии.
